...
public class LinAlg
{
	...
	public static <T extends Comparable> int max(T[] v) {
		int i, index;
		for (index = 0, i = 1; i < v.length; i++) {
			if (v[index].compareTo(v[i]) < 0) 
				index = i;
		}
		return index;
	}
	
	public static <T extends Comparable> int min(T[] v) {
		int i, index;
		for (index = 0, i = 1; i < v.length; i++) {
			if (v[index].compareTo(v[i]) > 0) 
				index = i;
	...
	/** add tow column: A colA-th column + B colB-th column
	 * 
	 * @param A Matrix
	 * @param B Matrix
	 * @param colA
	 * @param colB
	 * @return vector as result column
	 */
	public static double[] add(double[][] A, double[][] B, int colA, int colB) {
		int i;
		int n = Math.min(A.length, B.length);
		double[] C = new double[n];
		
		for ( i = 0; i < n; i++) 
			C[i] = A[i][colA] + B[i][colB];
		return C;
	}
	...
	/** multiplicates a vector with a vector <br>
	  * (Returns the product of the vector a and vector b)
	  * @param a Vector
	  * @param b Vector
	  * @return r vector [r1 = [a1 * b1, a2 * b2....]
	  */
	...
	/** multiplicate a matrix's col-th column with x;
	 * 
	 * @param A matrix
	 * @param col col-th colturm to multiplicate with x;
	 * @param x
	 * @return R matrix
	 */
	public static double[][] multiplyCol(double[][] A, int col, double x) {
		int i;
		int n = A.length;
		int m = A[0].length;
		double[][] R = new double[n][m];
		for (i = 0; i < n; i++)
			System.arraycopy(A[i], 0, R[i], 0, m);
		for (i = 0; i < n; i++) {
			R[i][col] = A[i][col] * x;
		}
		return R;
	 ...
	 * 
	 * @param A matirx
	 * @param col col-th column
	 * @param b mod
	 * @return R matrix after mod
	 */
	public static double[][] mod(double[][] A, int col, int b) {
		int i;
		int n = A.length;
		int m = A[0].length;
		double[][] R = new double[n][m];
	 ...
	 * @return R matrix after floor
	 */
	public static double[][] floor(double[][] A, int col) {
		int i;
		int n = A.length;
		int m = A[0].length;
		double[][] R = new double[n][m];
