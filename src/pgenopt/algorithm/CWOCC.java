...
public class CWOCC extends Optimizer {
	...
	public CWOCC(PGenOpt pgenopt) {
		...
		NumGen = getInputValueInteger("NumGen", 100000);
		NumPar = getInputValueInteger("NumPar", 100);
...
//				double[] PXn = neighbour(PX);
				Point PXn = (Point) PX.clone();
				PXn.setX(neighbour(PX));
				updateF(PXn);
				double PFn = PXn.getF(0);

				if (PFn < PX_best.getF(0)) {
					PX_best.setX(PXn.getX());
					PX_best.setF(0, PFn);

					if (PFn < P_X[minPF_index].getF(0)) {
						P_X[minPF_index].setX(PXn.getX());
	...
	protected double[] neighbour(Point s) {
		...
		for (i = 1; i <= dimCon; i++) {
			if (sn[i - 1] > ub[i - 1] || sn[i - 1] < lb[i - 1]) {
				sn[i - 1] = lb[i - 1] + RanGen.nextDouble()
						* (ub[i - 1] - lb[i - 1]);
			}
		}
		return sn;
